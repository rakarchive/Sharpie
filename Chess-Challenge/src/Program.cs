            ...
            if (args.Length == 1)
            {
                switch (args[0]) {
                    ...
                    case "bench":
                        ...
                        ulong nodes = 0;
                        long ms = 0;
                        foreach (string fen in BenchmarkFen) {
                            Timer timer = new(10000000, DEPTH);
                            Chess.Board board = new();
                            board.LoadPosition(fen);
                            Board engineBoard = new(new(board));
                            Stopwatch sw = Stopwatch.StartNew();
                            bot.Think(engineBoard, timer);
                            sw.Stop();
                            ms += sw.ElapsedMilliseconds;
                            nodes += bot.Nodes;
                            Console.WriteLine("FEN: {0} ... Nodes: {1}", fen, bot.Nodes);
                        }
                        Console.WriteLine("{0} nodes {1} nps", nodes, (int)(nodes / (ms / 1000f)));
                        break;
                }
            }
            else
